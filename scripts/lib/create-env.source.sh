...
cp "./scripts/$TARGET_ENV/env.txt" "$ENVFILELOCATION"
{
  ...
  echo "GOOGLE_CLIENT_ID=$(./scripts/uuid.sh)"
  echo "GOOGLE_CLIENT_SECRET=$(./scripts/uuid.sh)"
  echo "STRIPE_PUBLISHABLE_KEY=$(./scripts/uuid.sh)"
  echo "STRIPE_SECRET_KEY=$(./scripts/uuid.sh)"
  echo "WEBHOOK_SIGNING_SECRET=$(./scripts/uuid.sh)"
}  >> "$ENVFILELOCATION"
